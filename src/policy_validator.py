...
class ValidationResult(BaseModel):
    ...
    status: ValidationStatus = Field(
        description="Status validasi: APPROVED, REJECTED, atau PENDING_REVIEW"
    )
    ...
    minimum_age_required: int = Field(
        description="Usia minimum yang dibutuhkan untuk jenis akun ini"
    )
    ...
    recommendations: Optional[list[str]] = Field(
        default=None, description="Rekomendasi untuk nasabah jika ditolak"
    )
...
def check_document_validity(expiry_date: str) -> dict:
    ...
    try:
        ...
        if days_diff > 30:
            return {"is_valid": True, "days_until_expiry": days_diff, "status": "VALID"}
        elif days_diff > 0:
            return {
                "is_valid": True,
        ...
        return {"is_valid": False, "days_until_expiry": 0, "status": "INVALID_FORMAT"}
...
Lakukan validasi lengkap dan berikan keputusan beserta alasannya."""
    ...
    result = agent.invoke(
        {"messages": [message]},
        config={"callbacks": callbacks or [], "run_name": "validate_policy"},
    )
